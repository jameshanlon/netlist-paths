...
BOOST_AUTO_TEST_CASE(dtype_strings) {
  ...
  // dtype is reported as a logic vector. This appears to be a shortcoming in the XML format.
...
BOOST_AUTO_TEST_CASE(dtype_list) {
  ...
  // All the types.
  BOOST_TEST(np->getNamedDTypes().size() == 15);

  // Basic logic data type, expect to see just one.
  BOOST_TEST(np->getNamedDTypes("logic").size() == 1);

  // Top-scoped data types.
  BOOST_TEST(np->getNamedDTypes("packed_struct_t").size() == 1);
  BOOST_TEST(np->getNamedDTypes("packed_struct_nested_t").size() == 1);
  BOOST_TEST(np->getNamedDTypes("packed_struct_nested2_t").size() == 1);
  BOOST_TEST(np->getNamedDTypes("packed_struct_nested3_t").size() == 1);
  BOOST_TEST(np->getNamedDTypes("union_logic_t").size() == 1);
  BOOST_TEST(np->getNamedDTypes("enum_auto_t").size() == 1);
  BOOST_TEST(np->getNamedDTypes("enum_onehot_t").size() == 1);

  // Module-scoped data types.
  BOOST_TEST(np->getNamedDTypes("dtypes.packed_struct_t").size() == 1);
  BOOST_TEST(np->getNamedDTypes("dtypes.packed_struct_nested_t").size() == 1);
  BOOST_TEST(np->getNamedDTypes("dtypes.packed_struct_nested2_t").size() == 1);
  BOOST_TEST(np->getNamedDTypes("dtypes.packed_struct_nested3_t").size() == 1);
  BOOST_TEST(np->getNamedDTypes("dtypes.union_logic_t").size() == 1);
